...
1. **`data-pipeline/transform-data.ts`** — An offline, one-shot TypeScript script (run via `tsx`). Reads the full DC crime geojson, filters to `OFFENSE === "MOTOR VEHICLE THEFT"` Point features, and reduces each feature to just `CCN`, a computed `age` (days since `REPORT_DAT`), and `geometry`. This shrinking is the whole point: it cuts browser load time. To visualize a different crime type or set of fields, edit the `isPointTheft` predicate / `.map` here. Note `age` is computed at transform time relative to `Date.now()`, so it is baked into `data.json` and only refreshes when the pipeline is re-run.

2. **`app/`** — A Vite static site in TypeScript. `app/index.html` is the Vite entry; `app/src/crime.ts` is the bundled ESM module; `app/src/types.ts` holds the dataset interfaces; `app/public/` holds assets served at the web root (`data.json`, the vendored `leaflet-heatmap.js`). Build output lands in `app/dist/`.
   ...
   - `crime.ts` is the entry point: fetches `/data.json`, maps features to `{lat, lng, age}` points, builds an OpenStreetMap base layer + a `HeatmapOverlay`, and composes both into an `L.Map`.
